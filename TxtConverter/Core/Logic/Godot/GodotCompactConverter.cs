...
{
    // Regex Patterns
    // Используем якоря ^ и $, чтобы проверять строку целиком (как Java matches())
    ...
    private void OptimizeTree(List<GdNode> nodes)
    {
        ...
        while (i < nodes.Count)
        {
            ...
            if (count >= 3)
            {
                // Инициализируем свойства как копию OrderedDictionary, чтобы сохранить порядок
                ...
                {
                    // Явно указываем "null", если Type == null, как в Java
                ...
                // Добавляем Layout в конец (OrderedDictionary гарантирует это)
    ...
    private void PrintNode(GdNode node, string indent)
    {
        ...
        // OrderedDictionary гарантирует порядок итерации, как в LinkedHashMap
    ...
    private class GdNode
    {
        ...
        // Changed to OrderedDictionary to preserve insertion order (like LinkedHashMap in Java)
